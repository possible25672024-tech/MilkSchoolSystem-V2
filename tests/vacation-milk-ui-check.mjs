...
assert.equal(Object.keys(issueInput.signatures || {}).length, 0, "View must preserve compatible empty signatures until Media Sprint");
assert.equal(Array.isArray(issueInput.photos), true, "View must preserve a compatible photo array until Media Sprint");
assert.equal(issueInput.photos.length, 0, "View must preserve an empty photo array until Media Sprint");
