class VacationEvidenceManager {
    ...
    contextKey(roomId, academicYear, semester, issueDate, days) {
        const room = String(roomId || "").trim();
        const year = String(academicYear || "").trim();
        const term = String(semester || "").trim();
        const date = String(issueDate || "").trim();
        const dayCount = Number(days);
        if (!room || !year || !term || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !Number.isInteger(dayCount) || dayCount < 1) {
            throw this.error("VACATION_EVIDENCE_CONTEXT_INVALID", "ห้องเรียนหรือบริบทของหลักฐานนมช่วงปิดเทอมไม่ถูกต้อง");
        }
        return `vacation_${room}_${year}_${term}_${date}_${dayCount}`;
    }
    ...
    async setRecordContext({ roomId, academicYear, semester, issueDate, days, record = null, owners = [] } = {}) {
        this.ensureDependencies();
        const nextKey = this.contextKey(roomId, academicYear, semester, issueDate, days);
    ...
    selectOwner(ownerKey) {
        ...
        if (key && !this.owners.has(key)) throw this.error("VACATION_EVIDENCE_OWNER_INVALID", "นักเรียนที่เลือกไม่อยู่ในห้องที่เข้าสู่ระบบ");
    ...
    async addPhotos(files) {
        ...
        const countResult = this.policy.validatePhotoCount(this.photoReferences.length + this.legacyPhotos.length + list.length);
    ...
    async commitActiveSignature(receiverName = "") {
        ...
        if (!owner) throw this.error("VACATION_EVIDENCE_OWNER_REQUIRED", "กรุณาเลือกนักเรียนก่อนลงลายเซ็นผู้ปกครองหรือผู้รับนม");
        if (!this.signaturePad) throw this.error("SIGNATURE_PAD_NOT_READY", "ยังไม่พร้อมรับลายเซ็นนมช่วงปิดเทอม");
    ...
    async buildLegacyEvidence() {
        ...
        for (const reference of this.photoReferences) {
            ...
            if (!payload?.dataUrl) throw this.error("MEDIA_PAYLOAD_MISSING", "ไม่พบรูปถ่ายนมช่วงปิดเทอมที่จัดเก็บไว้", { mediaId: reference.mediaId });
        ...
        for (const key of this.owners.keys()) {
            ...
            if (stored?.reference) {
                ...
                if (!payload?.dataUrl) throw this.error("SIGNATURE_PAYLOAD_MISSING", "ไม่พบลายเซ็นผู้รับนมช่วงปิดเทอม", { mediaId: stored.reference.mediaId });
                sig = payload.dataUrl;
...
window.VacationEvidenceManagerClass = VacationEvidenceManager;
window.VacationEvidenceManager = new VacationEvidenceManager();